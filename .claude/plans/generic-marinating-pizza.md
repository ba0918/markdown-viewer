...
View/Raw
モード切り替え機能を実装した際、DocumentHeaderのテーマが現在のテーマに追従しないという問題が発生。
...
**なぜこれを修正するのか:**
ユーザーがテーマを変更した際に、DocumentHeaderのスタイル（背景色、文字色、タブ色）も自動的に追従する必要がある。現状ではlight
themeのスタイルのままになっている。
...
4. class名が `document-header-theme-light` → `document-header-theme-dark`
   に変わる
...
- 全6テーマ（light, dark, github, github-dark, solarized-light,
  solarized-dark）で動作確認
