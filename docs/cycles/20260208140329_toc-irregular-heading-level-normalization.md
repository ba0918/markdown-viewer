# ToC不正見出しレベル正規化

**Cycle ID:** `20260208140329`
**Started:** 2026-02-08 14:03:29
**Completed:** 2026-02-08 22:04:22
**Status:** 🟢 Completed

---

## 📝 What & Why

h1から始まらない不正な見出しレベル（h3→h3→h2など）のMarkdownでも、ToCが違和感なく表示されるように、見出しレベルを正規化する機能を実装。**親検出アルゴリズム**により、1レベル上の親が存在しない場合はh2に変換し、h1がない文書では視覚的違和感（縦線・インデント）をCSSで解消する。

## 🎯 Goals

- h3やh2から始まる文書でも自然なToC表示 ✅
- 親がいない子要素を検出し、h2に変換 ✅
- 親がいる子要素（h2の下のh3など）はそのまま保持 ✅
- 既存の正常な文書（h1から始まる）には影響なし ✅
- h1がない文書では縦線・インデントを非表示（CSS対応） ✅
- ツリー構造が崩れない堅牢な実装 ✅

## 📐 Design

### Architecture

```
services/toc-service.ts
  ↓
domain/toc/
  extractor.ts          # extractHeadings（変更なし）
  normalizer.ts         # NEW - 見出しレベル正規化ロジック
  normalizer.test.ts    # NEW - 正規化テスト
  tree-builder.ts       # REFACTOR - buildTocTree（正規化後のレベルで構築）
  tree-builder.test.ts  # MODIFY - ツリー構築テスト（正規化ケース追加）
  types.ts              # MODIFY - NormalizedHeading型追加
```

### Files to Change

```
src/
  domain/
    toc/
      normalizer.ts                 # NEW - レベル正規化ロジック
      normalizer.test.ts            # NEW - 正規化テスト（h3開始、h2開始等）
      tree-builder.ts               # REFACTOR - extractor.tsから分離、正規化対応
      tree-builder.test.ts          # NEW - ツリー構築テスト
      extractor.ts                  # MODIFY - buildTocTreeをtree-builder.tsに移動
      extractor.test.ts             # MODIFY - テスト分割（extractHeadingsのみ）
      types.ts                      # MODIFY - NormalizedHeading型追加

  services/
    toc-service.ts                  # MODIFY - normalizeHeadings統合
    toc-service.test.ts             # MODIFY - 正規化統合テスト追加
```

### Key Points

- **正規化アルゴリズム**: 親検出方式 - 1レベル上の親が存在するかチェック、親なしは h2 に変換
- **親の定義**: h1は親不要、h2はh1が必要、h3はh2が必要
- **CSS対応**: h1がない文書では縦線（`::before`）とインデント（`padding-left`）を非表示（`:not(:has(> .toc-level-1))`）
- **分離・責務**: normalizer.ts（正規化）、tree-builder.ts（ツリー構築）、toc.css（プレゼンテーション）
- **後方互換性**: h1から始まる正常な文書は正規化の影響なし
- **テスト駆動**: RED→GREEN→REFACTOR で実装（全163テスト通過）

### Implementation Strategy (Completed)

1. **Phase 1: normalizer.ts実装（TDD）** ✅
   - `normalizeHeadingLevels(headings: TocHeading[]): TocHeading[]`
   - 親検出アルゴリズム実装: `hasImmediateParent(level, index)` 関数で1レベル上の親チェック
   - テストケース: h1開始、dig.mdケース（h3→h3→h2）、h1+h3（h2欠け）、h2→h3→h2、空配列、h2開始、単一h3、text/id保持
   - 全8テスト追加・通過

2. **Phase 2: tree-builder.ts分離（Refactor）** ✅
   - extractor.tsから`buildTocTree`を分離済み（前セッションで完了）
   - tree-builder.tsに配置済み
   - 既存テスト全通過確認済み

3. **Phase 3: services統合** ✅
   - toc-service.tsで正規化を統合済み
   - `extractHeadings → normalizeHeadings → buildTocTree` の流れ実装済み
   - 統合テスト全通過（全163テスト）

4. **Phase 4: アルゴリズム書き換え（dig.mdケース対応）** ✅
   - 最小レベル正規化 → 親検出アルゴリズムに完全書き換え
   - dig.mdケース（h3→h3→h2）で視覚的違和感が発生したため方針変更
   - normalizer.ts完全書き換え、normalizer.test.ts全テスト更新

5. **Phase 5: CSS修正（視覚的違和感解消）** ✅
   - h1がない文書で縦線とインデントを非表示にするCSSルール追加
   - `:not(:has(> .toc-level-1))` セレクタで h1 の有無を検出
   - toc.cssに3つのルール追加（縦線非表示、h2/h3インデント削除）

## ✅ Tests (All Passed - 163 total)

### domain/toc/normalizer.test.ts ✅

- [x] h1から始まる正常な文書: 正規化なし（親あり要素は保持）
- [x] dig.mdケース（h3→h3→h2）: 全て親なし → 全てh2に変換
- [x] h1+h3（h2欠け）: h3は親(h2)なし → h2に変換
- [x] h2→h3→h2: h3は親(h2=A)あり → h3のまま保持
- [x] 空配列: 空配列を返す
- [x] h2から始まる文書: h2そのまま、h3は親ありで保持
- [x] 単一見出し（h3のみ）: h2に変換
- [x] id/text保持: レベルは変わるがid/textは保持される

### domain/toc/tree-builder.test.ts ✅

- [x] 正規化後のレベルでツリー構築
- [x] ルート要素が正しく配置される
- [x] 親子関係が正しく構築される
- [x] 空配列: 空ツリーを返す

### services/toc-service.test.ts ✅

- [x] 正規化統合テスト: 親なし要素の検出とh2変換
- [x] 既存の正常文書: 影響なし（既存テスト通過）

### UI/CSS ✅

- [x] h1がない文書で縦線非表示（toc.css）
- [x] h1がない文書でインデント削除（toc.css）
- [x] 全6テーマで正常表示確認

## 🔒 Security ✅

- [x] 入力検証: headingsが空でもクラッシュしない
- [x] 無限ループ防止: 正規化ロジックが確実に終了する（O(n)の線形処理）

## 📊 Progress

| Step | Status | Note |
|------|--------|------|
| normalizer.ts実装（TDD） | 🟢 | RED→GREEN→REFACTOR完了（最小レベル正規化） |
| tree-builder.ts分離（Refactor） | 🟢 | 既存テスト通過確認済み |
| services統合 | 🟢 | toc-service.ts修正完了 |
| Commit 1 | 🟢 | `[b3ed3fc]` feat: ToC不正見出しレベル正規化機能を実装 |
| dig.mdケース視覚的違和感発見 | 🟢 | h3→h3→h2で縦線が親なし子要素を示唆 |
| アルゴリズム書き換え | 🟢 | 最小レベル正規化 → 親検出アルゴリズムに完全書き換え |
| normalizer.test.ts更新 | 🟢 | 全8テスト更新・通過（親検出ロジック対応） |
| CSS修正 | 🟢 | h1なし文書で縦線・インデント非表示（toc.css） |
| テスト全通過確認 | 🟢 | 全163テスト通過 |
| Commit 2 | 🟢 | `[0159b3f]` fix: ToC生成時にFrontmatter除外済みcontentを使用 |
| Commit 3 | 🟢 | `[38468ee]` feat: ToC親検出アルゴリズムとCSS修正（h1なし文書の視覚的違和感を解消） |

**Legend:** ⚪ Pending · 🟡 In Progress · 🟢 Done

---

## 💡 Design Notes

### 正規化アルゴリズム（親検出方式）

```typescript
// normalizer.ts
export function normalizeHeadingLevels(headings: TocHeading[]): TocHeading[] {
  if (headings.length === 0) return [];

  /**
   * 指定されたインデックスの見出しに「1レベル上の親」が存在するかチェック
   */
  const hasImmediateParent = (level: number, index: number): boolean => {
    // h1 は親不要
    if (level === 1) {
      return true;
    }

    // 1レベル上の親レベル
    const parentLevel = level - 1;

    // それより前に parentLevel が存在するか？
    for (let i = 0; i < index; i++) {
      if (headings[i].level === parentLevel) {
        return true;
      }
    }

    return false;
  };

  // 親がいない場合は h2 に変換
  return headings.map((h, i) => {
    if (!hasImmediateParent(h.level, i)) {
      return { ...h, level: 2 as 1 | 2 | 3 };
    }
    return h;
  });
}
```

### CSS対応（h1なし文書の視覚的違和感解消）

```css
/* h1がない場合は縦線を非表示（親なし子要素の違和感を解消） */
.toc-list:not(:has(> .toc-level-1)) .toc-level-2::before,
.toc-list:not(:has(> .toc-level-1)) .toc-level-3::before {
  display: none;
}

/* h1がない場合はインデントも削除（全てフラット表示） */
.toc-list:not(:has(> .toc-level-1)) .toc-level-2 {
  padding-left: 0;
}

.toc-list:not(:has(> .toc-level-1)) .toc-level-3 {
  padding-left: 0;
}
```

### 具体例（親検出アルゴリズム）

#### ケース1: dig.mdケース（h3→h3→h2）

**入力:**
```
h3: Phase 2
h3: Phase 3
h2: Decisions
```

**正規化後（親検出）:**
```
h2: Phase 2    (h3 → h2) ← 親(h2)いない
h2: Phase 3    (h3 → h2) ← 親(h2)いない
h2: Decisions  (h2 → h2) ← 親(h1)いない、そのまま
```

**ツリー表示（フラット、縦線なし）:**
```
Phase 2
Phase 3
Decisions
```

#### ケース2: h2から始まる（親あり子あり混在）

**入力:**
```
h2: Introduction
h3: Overview
h3: Features
h2: Setup
```

**正規化後（親検出）:**
```
h2: Introduction  (h2 → h2) ← 親(h1)いない、そのまま
h3: Overview      (h3 → h3) ← 親(h2=Introduction)あり、保持
h3: Features      (h3 → h3) ← 親(h2=Introduction)あり、保持
h2: Setup         (h2 → h2) ← 親(h1)いない、そのまま
```

**ツリー表示（縦線なし、親ありh3は階層表示）:**
```
Introduction
  Overview
  Features
Setup
```

#### ケース3: h1から始まる（正常）

**入力:**
```
h1: Title
h2: Section 1
h3: Sub 1.1
```

**正規化後（親検出）:**
```
h1: Title       (h1 → h1) ← 親不要、そのまま
h2: Section 1   (h2 → h2) ← 親(h1=Title)あり、保持
h3: Sub 1.1     (h3 → h3) ← 親(h2=Section 1)あり、保持
```

**ツリー表示（通常、縦線あり）:**
```
Title
└─ Section 1
   └─ Sub 1.1
```

#### ケース4: h1+h3（h2欠け）

**入力:**
```
h1: Title
h3: Detail A
h3: Detail B
```

**正規化後（親検出）:**
```
h1: Title      (h1 → h1) ← 親不要、そのまま
h2: Detail A   (h3 → h2) ← 親(h2)いない
h2: Detail B   (h3 → h2) ← 親(h2)いない
```

**ツリー表示:**
```
Title
  Detail A
  Detail B
```

---

**Next:** Write tests → Implement → Commit with `smart-commit` 🚀
