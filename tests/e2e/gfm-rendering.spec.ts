...
test.describe('GitHub Flavored Markdown (GFM)', () => {
  ...
  test('タスクリストのスタイルが正しく適用される', async ({ page, testServerUrl }) => {
    ...
    const taskListItem = page.locator('li').filter({ has: page.locator('input[type="checkbox"]') }).first();
  ...
  test('テーブルが正しくレンダリングされる（GFM）', async ({ page, testServerUrl }) => {
    ...
    const strikethroughInTable = page.locator('table del:has-text("Old Feature")');
  ...
  test('テーブル内の複合GFM機能', async ({ page, testServerUrl }) => {
    ...
    // テーブル内の打ち消し線付きリンク
    const strikethroughLinkInTable = page.locator('table').locator('del:has-text("https://old.example.com")');
